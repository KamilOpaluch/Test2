javascript:(function(){
    ...
    function removeDuplicateHeaders(headers) {
        ...
        for (const header of headers) {
            ...
            let counter = 1;
            while (seen.has(modifiedHeader)) {
                modifiedHeader = `${header}_${counter}`;
                counter++;
            }
            seen.add(modifiedHeader);
            uniqueHeaders.push(modifiedHeader);
        }
    ...
    function collect(){
        ...
        if(t){
            ...
            if (data.length > 0) {
                // Process headers to remove duplicates
                data[0] = removeDuplicateHeaders(data[0]);
        ...
        if(grid){
            ...
            if(heads.length) {
                heads = removeDuplicateHeaders(heads);
                data2.push(heads);
            }
        ...
        for(const sc of scrollers){
            var rows2=Array.from(sc.querySelectorAll('div[tabindex], [data-row-index]'));
            if(rows2.length>5){
                var data4=[];
                // Check if first row might be headers
                if (rows2[0].querySelectorAll('div,span').length > 0) {
                    var headers = Array.from(rows2[0].querySelectorAll('div,span')).map(c=>S(c.innerText)).filter(Boolean);
                    if (headers.length) {
                        headers = removeDuplicateHeaders(headers);
                        data4.push(headers);
                        rows2 = rows2.slice(1); // Remove header row from data rows
                    }
                }
                rows2.forEach(r=>{
